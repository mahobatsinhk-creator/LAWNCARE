...
const PATCH_KEY = 'lawncare-spring-fall-video-2026';
...
foreach ($services as $index => $service) {
    if (($service['slug'] ?? '') !== 'spring-fall-clean-up') {
        continue;
    }
...
if (! $updated) {
    ...
    exit("spring-fall-clean-up service not found\n");
}

SiteContent::saveFormData('home', ['home_services' => $services]);

echo "Updated spring & fall cleanup service video to {$video}\n";
echo 'Delete update-spring-fall-cleanup-video.php now.' . PHP_EOL;
